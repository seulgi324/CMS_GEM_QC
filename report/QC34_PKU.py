...
def qc3_data(d3, label):    
    data_qc3 = pd.read_excel(r'/DATA/PATH/QC3_{}_{}.xlsx'.format(label, d3), engine='openpyxl')
    return data_qc3

def qc4_data(d4, label):    
    data_qc4 = pd.read_excel('/DATA/PATH/QC4_{}_{}.xlsx'.format(label, d4), engine='openpyxl', skiprows=[0, 2], na_values=['nan'])
    return data_qc4
...
def qc3_plot(mn, d3, label):
    ...
    fig.set_figheight(9)
    fig.set_figwidth(10)
    hep.cms.label(llabel="Preliminary", rlabel="PKU")
    data = qc3_data(d3, label)  
    time = np.array(np.round(data['Seconds'].tolist()))
    print(time)
    time_hr = time/3600
    ...
    t0 = int(np.where(np.round(time) == 0)[0])
    t1 = int(np.where(np.round(time) == 3540)[0])
    p0 = (26, 0.001)
...
def qc4_plot(mn, d4, label):
    ...
    voltage = np.array(dt['Vmon'].dropna().tolist()[:34]).astype(np.float)/1000
    print(voltage)
    current = np.array(dt['Imon'].dropna().tolist()[:34]).astype(np.float)
...
def qc34_report(mn, d3, d4, b, r_m, label):
    ...
    pdf.set_font('FreeSans', '', 10)
    pdf.cell(30, 12, '{:.1f} / {:.1f}'.format(temperature, atm), ln=1, align='R')
    pdf.set_font('FreeSansB', '', 10)
    pdf.cell(85)
    pdf.cell(70, 12, 'Pressure drop (mBar/0.98 hr)')
    pdf.set_font('FreeSans', '', 10)
